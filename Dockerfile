...
RUN apt-get update && apt-get install -y --no-install-recommends \
    libglib2.0-0 libsm6 libxext6 libxrender1 git wget \
    && rm -rf /var/lib/apt/lists/*

RUN pip install --no-cache-dir --upgrade pip

# Install CPU-only torch first so basicsr doesn't try to pull CUDA junk
RUN pip install --no-cache-dir \
    torch==2.3.1+cpu torchvision==0.18.1+cpu \
    --index-url https://download.pytorch.org/whl/cpu

# Install everything else; PIP_USE_PEP517 fixes basicsr’s legacy setup.py behavior
...
# Pull Real-ESRGAN inference script
